...
export const FaqUpdateSchema = z.object({
  id: z.string().uuid(),
  question: z.string().min(5, "Question must be at least 5 characters").max(500).optional(),
  answer: z.string().min(5, "Answer must be at least 5 characters").max(2000).optional(),
...
export const createFaq = createServerFn({ method: "POST" })
  .validator((d: unknown) => FaqCreateSchema.parse(d))
  .handler(async ({ data }) => {
    await requireAdminSession();
...
export const updateFaq = createServerFn({ method: "POST" })
  .validator((d: unknown) => FaqUpdateSchema.parse(d))
  .handler(async ({ data }) => {
    await requireAdminSession();
...
export const getProjectBySlug = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    try {
      ...
      // Use maybeSingle() so a missing row returns null instead of throwing (PGRST116).
...
// Public read — resolves a list of slugs to lightweight related-project cards
// from published read models. Used by the "Related Projects" section.
export const getRelatedProjects = createServerFn({ method: "GET" })
  .validator(z.object({ slugs: z.array(z.string()).max(12) }))
  .handler(async ({ data }) => {
    ...
    try {
      ...
      const { data: rows, error } = await sb
        .from("project_read_models")
        .select("slug, content_payload")
        .in("slug", data.slugs);

      if (error) throw new Error(error.message);

      // Preserve the admin-specified order of slugs.
      const bySlug = new Map((rows || []).map((r: any) => [r.slug, r.content_payload]));
      return data.slugs
        ...
        .map((payload: any) => {
          ...
          return {
            ...
            city: pm.location_district ? `${pm.location_district}, ${pm.city}` : pm.city,
            price: minPrice ? `₱${(Number(minPrice) / 1_000_000).toFixed(1)}M` : "",
...
export const deleteProjects = createServerFn({ method: "POST" })
  .validator(z.object({ ids: z.array(z.string().uuid()) }))
  .handler(async ({ data }) => {
    await requireAdminSession();
    const sb = await getServerClient();

    const { error } = await sb.from("projects").delete().in("id", data.ids);
...
export const PropertyCreateSchema = z.object({
  ...
  price_max: z.number().min(0).default(0),
  price_max_display: z.string().default(""),
...
export const PropertyUpdateSchema = z.object({
  ...
  price_max_display: z.string().optional(),
...
// Public submission — honeypot + timing guard, no auth required.
export const submitApplication = createServerFn({ method: "POST" })
  .validator((d: unknown) => JobApplicationSchema.parse(d))
  .handler(async ({ data }) => {
    assertAllowedOrigin();

    if (data.company) throw new Error("Invalid submission.");
    if (data.form_started_at && Date.now() - data.form_started_at < 2500)
      throw new Error("Invalid submission.");
...
// Admin read — all applications, newest first.
...
export const updateApplication = createServerFn({ method: "POST" })
  .validator((d: unknown) => ApplicationUpdateSchema.parse(d))
  .handler(async ({ data }) => {
    await requireAdminSession();
    const { id, ...updates } = data;
    const sb = await getServerClient();
